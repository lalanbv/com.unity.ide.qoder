...
namespace Microsoft.Unity.VisualStudio.Editor
{
    ...
    public class VisualStudioEditor : IExternalCodeEditor
    {
        ...
        private static string s_PackageDisplayName;
        ...
        {
            s_PackageDisplayName = "Qoder Editor";
            s_PackageVersion = "Unknown";

            try
            {
                var assembly = typeof(VisualStudioEditor).Assembly;
                var packageInfo = UnityEditor.PackageManager.PackageInfo.FindForAssembly(assembly);
                if (packageInfo != null)
                {
                    s_PackageDisplayName = packageInfo.displayName;
                    s_PackageVersion = packageInfo.version;
                    return;
                }

                var assemblyPath = assembly.Location;
                var editorDir = Path.GetDirectoryName(assemblyPath);

                string packageJsonPath = null;
                var currentDir = editorDir;
                ...
                if (string.IsNullOrEmpty(packageJsonPath))
                {
                    ...
                    {
                        currentDir = Path.GetDirectoryName(scriptPath);
                        for (int i = 0; i < 5 && currentDir != null; i++)
                        {
                            var candidate = Path.Combine(currentDir, "package.json");
                            if (File.Exists(candidate))
                ...
                if (!string.IsNullOrEmpty(packageJsonPath) && File.Exists(packageJsonPath))
                {
                    ...
                    if (jsonNode != null)
                    {
                        if (jsonNode["displayName"] != null)
                            s_PackageDisplayName = jsonNode["displayName"].Value;
                        if (jsonNode["version"] != null)
                            s_PackageVersion = jsonNode["version"].Value;
                    }
                }
            }
            catch (Exception ex)
            {
                Debug.LogWarning($"Failed to load package info: {ex.Message}");
            }
        }
        ...
        public void OnGUI()
        {
            ...
            GUILayout.Label($"<size=10><color=grey>{PackageInfoHelper.DisplayName} v{PackageInfoHelper.Version} enabled</color></size>", style);
            GUILayout.EndHorizontal();

            if (installation is VisualStudioQoderInstallation)
            {
                EditorGUILayout.Space();
                var reuseWindow = EditorPrefs.GetBool(VisualStudioQoderInstallation.ReuseExistingWindowKey, true);
                var newReuseWindow = EditorGUILayout.Toggle(
                    new GUIContent("Reuse existing Qoder window", "When enabled, opens files in an existing Qoder window if found. When disabled, always opens a new window."),
                    reuseWindow);
